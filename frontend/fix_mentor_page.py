...
export default function MentorPage() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (rmRes.status === "fulfilled" && rmRes.value) {
        ...
        const phases = rmRes.value.milestones;
  ...
  function renderMarkdown(text: string) {
    // Basic Markdown parser using marked, sanitized with DOMPurify
    // To ensure target="_blank" is used in links, we can use marked renderer or just rely on DOMPurify
    try {
